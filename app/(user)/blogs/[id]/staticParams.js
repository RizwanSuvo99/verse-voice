...
export async function generateStaticParams() {
  ...
  const staticIds = allBlogs.map(blog => ({
    id: blog.id.toString(),
  }));
  ...
  const maxExistingId = Math.max(...allBlogs.map(blog => blog.id), 0);
  ...
  console.log(`Generated ${allIds.length} static params for blog view routes`);
